...
export class FilmRequestHandler {
    ...
    private handleCreateError(err: CreateError, res: Response) {
        ...
        if (err instanceof FilmNrExists) {
            this.handleIsbnExists(err.filmNr, err.id, res);
        }
    }

    private handleIsbnExists(filmNr: string, id: string, res: Response) {
        const msg = `Die Film-Nummer "${filmNr}" existiert bereits bei ${id}.`;
